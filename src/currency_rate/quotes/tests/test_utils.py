...
from currency_rate.quotes.utils import get_currency_quote_vatcomply, get_or_create_currency_quotes_by_dates
...
def test_get_currency_quote_with_currency_code(vatcomply, currency_quote_usd):
    ...
    quotes = get_currency_quote_vatcomply(current_date, currency_code=currency_quote_usd.currency.code)
    assert len(quotes) == 1
    assert quotes[0]['currency_code'] == currency_quote_usd.currency.code
    assert quotes[0]['date'] == current_date
